	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'createWebRtcTransport':
			{
				...
				const { maxIncomingBitrate, maxOutgoingBitrate ,minOutgoingBitrate } = config.mediasoup.webRtcTransportOptions;
				...
				{
					try { await transport.setMaxOutgoingBitrate(maxOutgoingBitrate); }
					catch (error) {}
				}

				if (minOutgoingBitrate)
				{
					try { await transport.setMinOutgoingBitrate(minOutgoingBitrate); }
					catch (error) {}
				}
